        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.SearchTerm))
        {
            ...
            query = query.Where(x =>
                ...
                (
                    x.OldValues != null &&
                    x.OldValues.ToLower().Contains(searchTerm)
                ) ||
                (
                    x.NewValues != null &&
                    x.NewValues.ToLower().Contains(searchTerm)
                ) ||
                (
                    x.IpAddress != null &&
                    x.IpAddress.ToLower().Contains(searchTerm)
                ));
        }
        ...
        if (!string.IsNullOrWhiteSpace(request.EntityName))
        {
            ...
            query = query.Where(x =>
                x.EntityName.ToLower() == entityName);
        }
        ...
        {
            query = query.Where(x =>
                x.PerformedByUserId == request.PerformedByUserId.Value);
        }
        ...
        if (request.ToDate.HasValue)
        {
            ...
            query = query.Where(x =>
                x.PerformedAt < toDateExclusiveUtc);
        }
