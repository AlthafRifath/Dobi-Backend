...
namespace Dobi.Application.Features.Collections.GetCollections
{
    ...
    : IRequestHandler<GetCollectionsQuery, PagedResponse<OrderCollectionResponse>>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.SearchTerm))
            {
                ...
                query = query.Where(x =>
                    ...
                    (x.CollectorName != null && x.CollectorName.ToLower().Contains(searchTerm)) ||
                    (x.CollectorMobileNo != null && x.CollectorMobileNo.ToLower().Contains(searchTerm)));
            }

            if (request.CollectionModeId.HasValue)
            {
                query = query.Where(x => x.CollectionModeId == request.CollectionModeId.Value);
            }

            if (request.FromDate.HasValue)
            {
                var fromDate = request.FromDate.Value.ToDateTime(TimeOnly.MinValue);
                query = query.Where(x => x.CollectedOrDeliveredAt >= fromDate);
            }

            if (request.ToDate.HasValue)
            {
                var toDate = request.ToDate.Value.ToDateTime(TimeOnly.MaxValue);
                query = query.Where(x => x.CollectedOrDeliveredAt <= toDate);
            }
